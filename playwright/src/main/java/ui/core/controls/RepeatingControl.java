...
public class RepeatingControl<T> extends BaseControl {
	...
	public T get(int row) {
		...
		if (controlId == null) {
			...
			switch (locatorMethod) {
				case DATA_TEST_ID:
					return getControl.apply(getRowLocator(row).getByTestId(controlId));
				case TEXT:
					return getControl.apply(getRowLocator(row).getByText(controlId));
				case XPATH:
					return getControl.apply(getRowLocator(row).locator(controlId));
			}
		}
		return null;
	}
	...
	private Locator getRowLocator(int row) {
		...
		return locator.locator(String.format("%s[%d]", rowLocatorPattern, row));
	}

	private int getRowCount() {
		return locator.locator(rowLocatorPattern).all().size();
	}
}
